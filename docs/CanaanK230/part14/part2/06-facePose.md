...
- 默认为原图像尺寸 `rgb888p_size`，也可手动指定 `input_image_size`。

**仿射矩阵生成**：

- 调用 `get_affine_matrix(det)`，根据人脸框 bbox 生成 affine warp 所需的 2x3 仿射矩阵。

**设置 affine 参数**：

- 使用 `cv2_bilinear` 插值，背景填充为灰色（127）。

**构建流程图**：

- `build()` 接收输入 tensor 和输出 tensor 的 shape，用于预处理模块生成图像流。
...
使用一个缩放因子 `factor = 2.7`，适当扩大人脸区域以适配模型期望输入。

计算边界框中心点坐标

缩放比例 `scale = (edge_size * 2.0) / maximum_edge`：确保人脸完整显示在模型输入区域

偏移量 cx, cy 计算用于平移

构建仿射矩阵 `affine_matrix = [scale, 0, cx, 0, scale, cy]`，表示仿射变换的 2×3 矩阵
...
从模型输出（结果 tensor）中提取旋转矩阵并转换为欧拉角

- `results[0][0]`：通常表示模型输出的第一个 batch、第一个样本，即 3×3 的 rotation matrix（旋转矩阵）
- `get_euler()`：从 rotation matrix 中提取欧拉角（Pitch, Yaw, Roll）
...
将旋转矩阵 `R`（3×3）转化为欧拉角（pitch、yaw、roll）。

基于经典的三维旋转矩阵转欧拉角的公式，如果绕 `Y` 轴的旋转 sin 值 `sy` 过小，则认为姿态处于奇异状态（Gimbal lock），需特殊处理。
...
![person_122](${images}/person_122.png)

​	点击运行代码后，可以在显示屏上看到人脸姿态估计的结果。如下所示：

![image-20250423173014833](${images}/image-20250423173014833.png)
