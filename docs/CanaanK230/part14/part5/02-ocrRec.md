...
创建`PipeLine`实例，用于获取视频帧。

初始化`OCRDetRec`，开始图像的OCR检测和识别流程。

- **OCR检测**：首先使用`ocr_det`进行文本区域的检测。
- **OCR识别**：然后对每个检测到的文本框进行字符识别，得到OCR字符串。
...
![image-20250423183009683](${images}/image-20250423183009683.png)
