...
根据显示模式（`hdmi`或`lcd`）设置显示分辨率。

设置OCR模型路径、阈值和图像尺寸。

初始化`PipeLine`，并开始创建图像采集和显示管道。

实例化`OCRDetectionApp`对象，配置预处理方法。

进入循环，不断获取图像帧进行推理、后处理和绘制结果。

在循环内部，调用`pl.get_frame()`获取图像帧，使用`ocr_det.run()`执行推理，最后通过`ocr_det.draw_result()`绘制检测结果到显示设备。
...
![image-20250423182608052](${images}/image-20250423182608052.png)

​	运行代码后，可以在显示屏看到检测结果：

![image-20250423182723177](${images}/image-20250423182723177.png)
