...
```
Display.show_image(img, x=round((480-sensor.width())/2), y=round((360-sensor.height())/2))
```

将图像显示在屏幕中央（屏幕为 480×360时）。
...
try:
    ...
    while True:
        ...
        Display.show_image(
            img,
            x=round((480 - sensor.width()) / 2),
            y=round((360 - sensor.height()) / 2)
        )
