...
创建一个 `Sensor` 对象，分辨率为 `480x360`，开始进行视频采集准备。
...
```
Display.init(Display.AML020T, width=DETECT_WIDTH, height=DETECT_HEIGHT, fps=60, to_ide=True)
```

初始化虚拟显示器（AML020T 模式），支持在 IDE 显示图像；
...
![image-20260605110954044](images/image-20260605110954044.png)